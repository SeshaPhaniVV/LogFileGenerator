...
object Parameters:
   ...
   implicit private val comp: Ordering[Double] = new Ordering[Double] {
     def compare(x: Double, y: Double) =
       if math.abs(x - y) <= COMPARETHREASHOLD then 0
       else if x - y > COMPARETHREASHOLD then -1
       else 1
   }
   ...
   private def timeoutRange: (Long, Long) =
      val lst = Try(config.getLongList(s"randomLogGenerator.TimePeriod").asScala.toList) match {
        ...
        case Failure(exception) =>
          logger.error(s"No config parameter Timeout is provided")
          throw new IllegalArgumentException(s"No config data for Timeout")
      }
      if lst.length != 2 then
         throw new IllegalArgumentException(s"Incorrect range of values is specified for Timeout")
      (lst(0), lst(1))
   end timeoutRange
   ...
   private def logMsgRange(logTypeName: String): (Double, Double) =
      val lst = Try(
        config.getDoubleList(s"randomLogGenerator.logMessageType.$logTypeName").asScala.toList
      ) match {
        case Success(value) => value.sorted
        case Failure(exception) =>
          logger.error(s"No config parameter is provided: $logTypeName")
          throw new IllegalArgumentException(s"No config data for $logTypeName")
      }
      if lst.length != 2 then
         throw new IllegalArgumentException(
           s"Incorrect range of values is specified for log $logTypeName"
         )
      (lst(0), lst(1))
   end logMsgRange
   ...
   private def func4Parameter[T](defaultVal: T, f: String => T): String => T =
     (pName: String) =>
       Try(f(s"randomLogGenerator.$pName")) match {
         case Success(value) => value
         case Failure(exception) =>
           logger.warn(s"No config parameter $pName is provided. Defaulting to $defaultVal")
           defaultVal
       }
   end func4Parameter
   ...
   private def getParam[T](pName: String, defaultVal: T): ConfigType2Process[T] =
     defaultVal match {
       ...
       case v: (Double, Double) => logMsgRange(pName)
     }
   end getParam
   ...
   val runDurationInMinutes: Duration =
     if Parameters.maxCount > 0 then Duration.Inf else getParam("DurationMinutes", 0).minutes

   if Parameters.maxCount > 0 then
      logger.warn(s"Max count ${Parameters.maxCount} is used to create records instead of timeouts")
   if timePeriod._1 < 0 || timePeriod._2 < 0 then
      throw new IllegalArgumentException("Timer period cannot be less than zero")
   ...
   private val compFunc: ((Double, Double), (Double, Double)) => Boolean =
     (input1, input2) => (input1._1 <= input2._1) && (input1._2 < input2._2)
   val logRanges = ListMap(
     Map[(Double, Double), String => Unit](
   ...
   if checkForNestedRanges(logRanges) then
      throw new Exception("Overrlapping likelihood ranges will lead to the loss of precision.")
